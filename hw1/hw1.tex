  ...
  \item In each case below, use the Master Theorem (MT) to provide tight asymptotic bounds for the
    ...
    \begin{solution}
      \begin{enumerate}
        ...
        \item
          First, assume $a > b$. Then $\log_b a > 1$. Therefore, we can write $\log_b a = 1 + \varepsilon$ for some $\varepsilon>0$, and $dn = O( n^{\log_b a - \varepsilon})$. Then by the Master Theorem, $T(n) = \Theta( n^{\log_b a} )$.
      \end{enumerate}<++>
